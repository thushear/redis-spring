 ...
 * @author  Rod Johnson
 * @since 18 April 2001
 * @version $Id: PropertyVetoExceptionsException.java,v 1.1.1.1 2002/08/01 12:52:02 Rod Johnson Exp $
 */
public class PropertyVetoExceptionsException extends BeansException {
	...
	public Iterator iterator() {
		return new Iterator() {
			...
			public Object next() {
				if (!hasNext())
					throw new NoSuchElementException("PropertyVetoExceptionExceptions iterator has only "
							+ getExceptionCount() + " elements");
				return exceptions.get(count++);
			}

			public void remove() {
				throw new UnsupportedOperationException(
						"Cannot remove elements from PropertyVetoExceptionExceptions iterator");
			}
		};
	} // iterator
	...
	void addMissingFields(InvalidPropertyValuesException ex) {
		...
		for (int i = 0; i < ex.getMissingFields().size(); i++) {
			addPropertyVetoException(new ErrorCodedPropertyVetoException(getBindObject(),
					(InvalidPropertyValuesException.MissingFieldException) ex.getMissingFields().get(i)));
		}
	}
