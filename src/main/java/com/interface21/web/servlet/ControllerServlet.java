...
public class ControllerServlet extends FrameworkServlet {
    ...
    protected void initFrameworkServlet() throws ServletException {
        ...
        if (handlerMaps.size() == 0) {
            logger.info("No HandlerMaps found in servlet '" + getServletName() + "': using default");
    ...
    protected void doService(HttpServletRequest request, HttpServletResponse response, boolean debugMode) throws ServletException, IOException {
        logger.info("ControllerServlet with name '" + getServletName() + "' received request for [" + request.getServletPath() + "]");
        ...
        if (mappedHandler == null) {
            ...
            logger.warn("No mapping for [" + request.getServletPath() + "] in ControllerServlet with name '" + getServletName() + "'");
    ...
    private boolean wasRevalidated(HttpServletRequest request, HttpServletResponse response, HandlerAdapter ha, Object mappedHandler) {
        ...
        logger.warn("GET request: " + HEADER_IFMODSINCE + " request header present");
        ...
        if (ifModifiedSince < (lastModified / 1000 * 1000)) {
            ...
            logger.warn("GET request: " + HEADER_IFMODSINCE + " request header contains an earlier date than lastModified date. Will regenerate content and reset lastModified header.");
            ...
            logger.warn("GET request: " + HEADER_IFMODSINCE + " request header contains a later date than lastModified date, or revalidation isn't supported. Indicating unmodified.");
  ...
  */
    ...
    private void initHandlerAdapter(HandlerAdapter ha, String beanName) throws ServletException {
        try {
            logger.info("Command servlet '" + getServletName() + "': Initializing HandlerAdapter object with name [" + beanName + "]: " + ha);
    ...
    private void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (mv.isReference()) {
            ...
            if (isDebugMode(request)) {
                ...
                logger.warn("Debug request attribute set: setting viewName of '" + viewName + "' as a request attribute");
